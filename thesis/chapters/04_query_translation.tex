...
Similarly to the parser for entities and mapping, the query parser interface defines only two methods, as demonstrated in Listing~\ref{lst:qparser}. The \texttt{CanParse} method receives a content kind descriptor together with the language of the source and indicates whether the parser is capable of processing it. The \texttt{Parse} method initiates the actual process and accepts both the entity mapping intermediate representation and the language hint so that parsers can handle dialect-specific syntax.

 \begin{lstlisting}[caption={IQueryParser interface structure}, language=pseudo, label={lst:qparser}]
interface IQueryParser

public:
    CanParse(contentKind, language) : boolean;
    Parse(source, entityMap, language);
 \end{lstlisting}
...
\begin{algorithm}[!p]
    ...
    \ForEach{\textup{$parser$ in $parsers$}}{
        \If{\textup{$queryBuilder$ == null $\land$ $parser$ is IQueryParser}}{\Continue}
        \ForEach{\textup{$source$ in $sources$}}{
            $kind$ = ContentKindRegistry.TryGet($source$.ContentKindId)\;
            \If{\textup{$kind$ == null}}{\Continue}
            \If{\textup{$parser$.CanParse($kind$, $source$.Language)}}{
                \If{\textup{$parser$ is IQueryParser}}{
                    $parser$.Parse($source$.Content, $entityBuilder$.EntityMap, $source$.Language)\;
                    $queryParsed$ = true\;
                } \Else {
                    $parser$.Parse($source$.Content, $source$.Language)\;
                }
                \Break
            }
        }
    ...
    \If{\textup{$queryBuilder$ \neq null $\land$ $queryParsed$}} {
